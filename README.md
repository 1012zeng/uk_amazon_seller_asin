...
类型检查和 22 个测试文件（120 项）通过；测试覆盖 B 列链接/纯 ID/重复/非法行、评分评论 NA、价格和日期边界、FBA/变体、全量七天销量路由、精确 `yes`/`No`、协议暂停、v15 SQLite、最终 `yes` 不变量和三张表导出。真实小样需单独记录实际覆盖到的阶段；若样本没有合格产品，不得据此宣称七天销量或详情链路已完成。
