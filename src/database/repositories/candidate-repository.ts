...
export interface FilterCandidateRow {
  ...
  price_pence: number; review_count: number; rating: number; enriched_image_url: string; child_sales_30d: number | null;
...
export class CandidateRepository {
    ...
    return this.db.transaction(() => {
      ...
      this.db.prepare(`WITH eligible AS (
        ...
        WHERE s.status='success' AND s.validation_state IN ('exact','verified_drift')
          AND o.rating IS NOT NULL AND o.rating${rating.eligibleOperator}? AND o.review_count IS NOT NULL AND o.review_count<=? AND o.price_pence IS NOT NULL AND o.price_pence BETWEEN ? AND ?
  ...
  private prefilterCounts(): { occurrenceCount: number; qualifiedOccurrenceCount: number; excludedStoreOccurrences: number; eligibleOccurrences: number; uniqueCandidates: number } {
    ...
    const eligibleOccurrences = (this.db.prepare(`SELECT COUNT(*) count FROM store_asin_occurrences o JOIN source_stores s USING(seller_id) WHERE s.status='success' AND s.validation_state IN ('exact','verified_drift') AND o.rating IS NOT NULL AND o.rating${rating.eligibleOperator}? AND o.review_count IS NOT NULL AND o.review_count<=? AND o.price_pence IS NOT NULL AND o.price_pence BETWEEN ? AND ?`).get(rating.threshold, f.maxReviewCount, f.minPricePence, f.maxPricePence) as CountRow).count;
  ...
  prefilterReasonCounts(): Record<string, number> {
    ...
    const rows = this.db.prepare(`SELECT reason,COUNT(*) count FROM (
      SELECT CASE
        ...
        WHEN o.rating IS NULL THEN 'rating_missing'
        WHEN o.rating${rating.rejectedOperator}? THEN '${rating.rejectedReason}'
        WHEN o.review_count IS NULL THEN 'review_count_missing'
        WHEN o.review_count>? THEN 'review_count_gt_max'
