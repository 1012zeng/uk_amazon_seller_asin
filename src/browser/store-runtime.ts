...
export class StoreBrowserRuntime {
  ...
  async start(): Promise<void> {
    ensureStoreProxyGeneration(this.config);
  ...
  private async performRecovery(cause: unknown): Promise<void> {
    ...
    try { reportStoreProxyFailure(this.config, this.config.stores.proxyPorts[this.proxyIndex]!, truncateError(cause)); } catch (error) {
