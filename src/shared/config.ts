...
export function loadConfig(configFile = "config/amazon-uk.local.yaml"): AppConfig {
  ...
  const historyFilter = object(raw.historyFilter ?? { enabled: false }, "historyFilter");
  ...
  const config: AppConfig = {
    ...
    source: {
      ...
      format: string(source.format ?? "seller_ids_b", "source.format") as SourceFormat,
    },
  ...
  if (expected.maxReviewCount !== 300 || expected.minRatingInclusive !== 3.5 || expected.minPricePence !== 699 || expected.maxPricePence !== 5000 || expected.maxVariations !== 3
    || expected.maxNewAgeDays !== 30 || expected.maxAgeDays !== 180 || expected.newListingDailySalesMinimum !== 3) {
