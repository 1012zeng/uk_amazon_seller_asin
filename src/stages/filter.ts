...
function childSalesRejection(row: FilterCandidateRow, age: number, config: ReturnType<RunStore["getRunConfig"]>): string {
  const filters = config.filters;
  const band = age <= filters.firstChildSalesBandMaxAgeDays
    ? { lower: 0, upper: filters.firstChildSalesBandMaxAgeDays, minimum: filters.firstChildSalesBandMinimum }
    : age <= filters.secondChildSalesBandMaxAgeDays
      ? { lower: filters.firstChildSalesBandMaxAgeDays + 1, upper: filters.secondChildSalesBandMaxAgeDays, minimum: filters.secondChildSalesBandMinimum }
      : { lower: filters.secondChildSalesBandMaxAgeDays + 1, upper: filters.maxAgeDays, minimum: filters.thirdChildSalesBandMinimum };
  const sales = row.child_sales_30d;
  if (sales === null || sales < band.minimum) return `child_sales_30d_lt_${band.minimum}_for_age_${band.lower}_${band.upper}`;
  return "";
}
  ...
  try {
    ...
    store.db.transaction(() => {
      for (const row of rows) {
        ...
        const age = listingAgeDays(row.available_date, asOfDate)!;
        const hasChildSales = row.child_sales_30d !== null;
        const needsSales7d = age <= config.filters.maxNewAgeDays && !hasChildSales;
        if (!needsSales7d) {
          const reason = childSalesRejection(row, age, config);
          if (reason) {
            store.candidates.setState(row.asin, "filtered_child_sales", "filter", reason, age);
            store.exports.remove(row.asin);
            continue;
          }
        }
