...
async function main(): Promise<number> {
  ...
  const configPath = option(args, "--config") ?? "config/amazon-uk-asin-links.yaml";
  if (resumeRunId && args.includes("--config")) throw new UsageError("ASIN 链接流水线只能使用 --resume 或 --config，不能同时使用");
