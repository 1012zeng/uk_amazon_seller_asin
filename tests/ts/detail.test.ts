...
async function seedFinalProducts(store: RunStore, count: number): Promise<string[]> {
  ...
  expect(await runSales7dStage(store, { health: async () => undefined, sales7d: async (asin, asOfDate, dailySalesMinimum) => ({ marketplace: "UK", asin, dataType: "prediction", ...fixedSales7dWindow(asOfDate), dailySalesMinimum, complete: true, result: "yes", days: [] }) })).toBe(0);
  return asins;
}
