...
export function runFixture(options: { sourceFormat?: SourceFormat; products?: SourceProductLink[] } = {}): { store: RunStore; config: AppConfig; root: string } {
  ...
  const config: AppConfig = {
    ...
    stores: { concurrency: { initial: 3, min: 1, max: 3, successWindowPages: 10, cooldownMs: 60000 }, requestsPerSecond: 2, maxResponseBytes: 5_242_880, maxRetries: 2, maxBlockedAttempts: 2, blockedDelayMs: 0, maxPagesPerStore: 0, paginationValidation: { ...DEFAULT_PAGINATION_POLICY }, proxyPorts: [1, 2, 3, 4], proxyControllerPath: root },
  ...
  const activeSeeds = sourceFormat === "asin_links_b" ? [] : seeds;
